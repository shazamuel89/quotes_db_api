<?php
    ...
    if ($resultArr['success'] === false) {                      // If read_single() failed
        ...
        die(json_encode([                                       // Kill script while displaying a json encoded array
            'message' => 'Author lookup failed.',               // With a user readable message
    ...
    $author_arr = $resultArr['data']->fetch(PDO::FETCH_ASSOC);  // Fetch the result of read_single() as an associative array (or false if no rows)
